...
def fillMissingInfo(file_in, file_out):
    ...
    while i < len(attributes_all):
        ...
        if match:
            ...
            file_contents[i] = re.sub(attributes_all[i], "", file_contents[i])
            if i < 6:
                file_contents[i] = re.sub("[^0-9]", "", file_contents[i])  # remove all non-numerical characters
            if i < 9:
                file_contents[i] = re.sub(" ", "", file_contents[i])
            if i == 3 or i == 5:  # remove trailing characters (cable types, amp usage, etc)
                                  # only want quantity of cables
                file_contents[i] = file_contents[i][0:1]
...
def loopThroughPages(my_url, currPage, totalPageAmount):  # total page amount dependent on webpage, be sure to check
    # when link changes
    while currPage <= totalPageAmount:
        ...
        for container in containers:
            ...
            # ------------------------------------------------------------------------------
            # single same tag values found from web scrape
            # ------------------------------------------------------------------------------
            print("Name: " + name)
            name = re.sub(",", " ", name)
            ...
            price = price[(price.find("$") + 1):(price.find(".") + 3)]
            print("Price: " + price)
            price = re.sub(",", "", price)
            file.write("Price: " + price + "\n")  # attr for in_file
            # ------------------------------------------------------------------------------
            # multiple same tag values found in web scrape
            # ------------------------------------------------------------------------------
            try:
...
loopThroughPages("https://www.newegg.com/p/pl?d=graphics+cards&page=1", 1, 4)
